...
#define SOC_PORT_ADDR_OR_NONE(nodelabel)					\
	IF_ENABLED(DT_NODE_EXISTS(DT_NODELABEL(nodelabel)),			\
		   (DT_REG_ADDR(DT_NODELABEL(nodelabel)),))

/** SAM0 port addresses */
static const uint32_t soc_port_addrs[] = {
	SOC_PORT_ADDR_OR_NONE(porta)
	SOC_PORT_ADDR_OR_NONE(portb)
	SOC_PORT_ADDR_OR_NONE(portc)
	SOC_PORT_ADDR_OR_NONE(portd)
	SOC_PORT_ADDR_OR_NONE(porte)
	SOC_PORT_ADDR_OR_NONE(portf)
};
...
static void pinctrl_configure_pin(pinctrl_soc_pin_t pin)
{
	...
	__ASSERT_NO_MSG(port_idx < ARRAY_SIZE(soc_port_addrs));
	port_func = SAM_PINMUX_FUNC_GET(pin);

	soc_pin.regs = (port_group_registers_t *) soc_port_addrs[port_idx];
